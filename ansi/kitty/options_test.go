...
func TestOptions_UnmarshalText(t *testing.T) {
	tests := []struct {
		...
		{
			name: "unmarshal transmission controls",
			text: []byte("f=100,s=320,v=240,o=z,t=f,S=4096,O=8,p=12,I=3"),
			want: Options{
				Format:       PNG,
				ImageWidth:   320,
				ImageHeight:  240,
		...
		{
			name: "unmarshal placement controls",
			text: []byte("x=1,y=2,z=-3,X=4,Y=5,c=80,r=24,P=6,Q=7"),
			want: Options{
				X:                 1,
				Y:                 2,
				Z:                 -3,
